...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for ghost in livingGhostPositionDistributions:
      for pos in ghost:
        if ghost[pos] > maxProb:
          maxProb = ghost[pos]
          possiblePosition = [pos]
        elif ghost[pos] == maxProb:
          possiblePosition.append(pos) 

    for action in legal:
      newPacmanPosition = Actions.getSuccessor(pacmanPosition, action)
      for pos in possiblePosition:
        minAct[self.distancer.getDistance(pos,newPacmanPosition)] = action
    return minAct[min(minAct)]
